...
void ChessGame::checkRomps(const Chess::Side& side)
{
   if (m_speedupRomps && m_moves.size() > 2)
   {
       int ply = m_moves.size() - 1;
       m_speedupRomps->update(ply, m_scores[ply], m_scores[ply-1]);
...
void ChessGame::correctMovesRate()
{
   if (!m_limitMovesRate)
       return;
   ...
   if (lastMoveTime < minMoveTimeMs) {
       auto delay = ulong(minMoveTimeMs - lastMoveTime);
...
void ChessGame::setSpeedupRomps(SpeedupRomps* speedupRomps)
{
   if (m_speedupRomps)
       delete m_speedupRomps;
   m_speedupRomps = speedupRomps != nullptr
       ? new SpeedupRomps(*speedupRomps) : nullptr;
}

void ChessGame::limitMovesRate(bool isLimit)
{
   m_limitMovesRate = isLimit;
}
